...
for epoch in range(num_epoch):
    ...
    h_real = torch.ones(batch_size,1)*h_real
    h_imag = torch.ones(batch_size,1)*h_imag

    # discriminator
    real_data = Variable(torch.cat([torch.Tensor(real_x),torch.FloatTensor(x_train),h_real,h_imag],1))
    ...
    z = Variable(torch.cat((z_noise,z_coding,h_real,h_imag),1))
    fake_data = G(z)
    fake_out = D(torch.cat([fake_data,torch.FloatTensor(x_train),h_real,h_imag],1))
    ...
    z = Variable(torch.cat([z_noise,z_coding,h_real,h_imag],1))
    output = G(z)
    fake_out = D(torch.cat([output,torch.FloatTensor(x_train),h_real,h_imag],1))
