...
class generator(nn.Module):
    def __init__(self, M, n, z_dimension):
        ...
        self.g = nn.Sequential(
            nn.Linear(2*n+z_dimension+2, 32),
            nn.ReLU(),
            nn.Linear(32, 32),
            nn.ReLU(),
            nn.Linear(32, n*2),
...
class discriminator(nn.Module):
    def __init__(self, M, n):
        ...
        self.d = nn.Sequential(
            nn.Linear(2*n+z_dimension+2, 16),
            nn.ReLU(),
            nn.Linear(16, 16),
            nn.ReLU(),
            nn.Linear(16, 1),
...
def channel_pilot(x, k, n, SNR, num_samples, h_real, h_imag):
    pilot = np.ones([num_samples, n])
    pilots = np.hstack([pilot*h_real, pilot*h_imag])
    noise = np.random.randn(num_samples, 2*n)*np.sqrt(1/(2*SNR*k/n))
    pilots = pilots + noise
    return pilots
...
for epoch in range(num_epoch):
    ...
    real_x, h_real, h_imag = channel_Rayleigh(x_train,k,n,SNR,batch_size)
    h_real_tensor = torch.ones(batch_size,1)*h_real
    h_imag_tensor = torch.ones(batch_size,1)*h_imag
    pilot = channel_pilot(x_train,k,n,SNR,batch_size,h_real,h_imag)
    pilot_tensor = torch.Tensor(pilot)

    # discriminator
    real_data = Variable(torch.cat([torch.Tensor(real_x),torch.FloatTensor(x_train),pilot_tensor],1))
    ...
    z = Variable(torch.cat((z_noise,z_coding,pilot_tensor),1))
    fake_data = G(z)
    fake_out = D(torch.cat([fake_data,torch.FloatTensor(x_train),pilot_tensor],1))
    ...
    z = Variable(torch.cat([z_noise,z_coding,pilot_tensor],1))
    output = G(z)
    fake_out = D(torch.cat([output,torch.FloatTensor(x_train),pilot_tensor],1))
...
while(1):
    h_real = random.normalvariate(0,1)/np.sqrt(2)
    h_imag = random.normalvariate(0,1)/np.sqrt(2)
    if abs(h_real)<1 and abs(h_imag)<1 and abs(h_real)>0.3 and abs(h_imag)>0.3:
        print(h_real,h_imag)
        break
h_real_tensor = torch.ones(num_samples,1)*h_real
h_imag_tensor = torch.ones(num_samples,1)*h_imag
...
for i in range(M):
    code = encode(M,i)
    after_channel = channel_Rayleigh_test(np.tile(code,(num_samples,1)),k,n,SNR,num_samples,h_real,h_imag)
